...
let portfolio = JSON.parse(localStorage.getItem('etf_portfolio') || '[]');
let userSettings = JSON.parse(localStorage.getItem('etf_settings') || '{}');
let currentUser = JSON.parse(localStorage.getItem('etf_user') || 'null');
...
function saveSettings() {
    localStorage.setItem('etf_settings', JSON.stringify(userSettings));
}

function addToPortfolio(etf, shares, avgPrice) {
    const existing = portfolio.find(h => h.isin === etf.isin);
    if (existing) {
        ...
        portfolio.push({
            isin: etf.isin, ticker: etf.ticker, name: etf.name,
            shares, avgPrice, currentPrice: etf.price,
            asset: etf.asset, region: etf.region
        });
    }
    savePortfolio();
...
function removeFromPortfolio(isin) {
    portfolio = portfolio.filter(h => h.isin !== isin);
    savePortfolio();
}

function computePortfolioMetrics() {
    if (portfolio.length === 0) return { totalValue: 0, invested: 0, pl: 0, plPercent: 0 };
    const totalValue = portfolio.reduce((s, h) => s + h.shares * h.currentPrice, 0);
    const invested = portfolio.reduce((s, h) => s + h.shares * h.avgPrice, 0);
    const pl = totalValue - invested;
    return { totalValue, invested, pl, plPercent: invested > 0 ? (pl / invested) * 100 : 0 };
}
...
function getAllocationByAsset() {
    ...
    const colors = { 'Azionario': '#0A2540', 'Obbligazionario': '#F59E0B', 'Commodities': '#EAB308', 'Monetario': '#84CC16', 'Altro': '#8A94A6' };
    const metrics = computePortfolioMetrics();
    return Object.entries(byAsset).map(([name, val]) => ({
        name, value: Math.round((val / metrics.totalValue) * 1000) / 10,
        color: colors[name] || '#8A94A6'
...
function createSectionHeader(title, description, action) {
    return `
        <div class="section-header">
            <div>
                <div style="display:flex;align-items:center;gap:12px;margin-bottom:4px;">
                    <div style="width:4px;height:28px;border-radius:2px;background:${COLORS.GREEN_LIGHT};"></div>
...
function createCard(content, extraClass = '') {
    return `<div class="card ${extraClass}"><div class="card-body">${content}</div></div>`;
}
...
function riskIndicator(level) {
    ...
    for (let i = 1; i <= 5; i++) {
        html += `<div class="risk-segment ${i <= level ? 'active' : ''}"></div>`;
    }
    html += '</div>';
    return html;
}

// Lucide icon SVGs (simplified)
const ICONS = {
    ...
    wallet: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12V7H5a2 2 0 0 1 0-4h14v4"></path><path d="M3 5v14a2 2 0 0 0 2 2h16v-5"></path><path d="M18 12a2 2 0 0 0 0 4h4v-4Z"></path></svg>`,
    ...
    'git-compare': `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path></svg>`,
    ...
    'book-open': `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"></path><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"></path></svg>`,
    ...
    bell: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"></path><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"></path></svg>`,
    ...
    'trending-up': `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"></polyline><polyline points="17 6 23 6 23 12"></polyline></svg>`,
    ...
    'pie-chart': `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21.21 15.89A10 10 0 1 1 8 2.83"></path><path d="M22 12A10 10 0 0 0 12 2v10z"></path></svg>`,
    ...
    'eye-off': `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94L9 9l3 3 3 3 2.94 2.94z"></path><path d="M9 9l3 3 3 3 2.94 2.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94L9 9z"></path><line x1="1" y1="1" x2="23" y2="23"></line></svg>`,
    'more-horizontal': `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="1"></circle><circle cx="19" cy="12" r="1"></circle><circle cx="5" cy="12" r="1"></circle></svg>`,
    'arrow-up-right': `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="7" y1="17" x2="17" y2="7"></line><polyline points="7 7 17 7 17 17"></polyline></svg>`,
    ...
    coins: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="8" cy="8" r="7"></circle><path d="M19.5 9.94A7 7 0 0 1 20 16"></path><path d="M22 16a7 7 0 0 1-2.5 5.4"></path></svg>`,
    'building-2': `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 22V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v18"></path><path d="M6 12H4a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6a2 2 0 0 0-2-2h-2"></path><path d="M6 8h12"></path><path d="M6 16h12"></path><path d="M9 2v14"></path><path d="M15 2v14"></path></svg>`,
    ...
    'credit-card': `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"></rect><line x1="1" y1="10" x2="23" y2="10"></line></svg>`,
    ...
    'menu-grid': `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"></rect><rect x="14" y="3" width="7" height="7"></rect><rect x="3" y="14" width="7" height="7"></rect><rect x="14" y="14" width="7" height="7"></rect></svg>`,
